...
class OrderBook(ABC):
    """Clase base abstracta para libros de órdenes.
    ...
    _sell_margin_pct: float = 1.0
    ambiguous_fills_count: int = 0
    ...
    def _check_open_guards(self, wallet: Wallet) -> Optional[str]:
        ...
        if slot > wallet.get_usd_balance() + 1e-6:
    ...
    def _execute_with_guards(
        ...
        # Marcar dirección y signal_type
        if order.trade:
...
class BaseSimulatedOrderBook(OrderBook):
    """Clase base simulada con lógica compartida de ejecución (compra/venta).
    NO usar directamente como modo mercado — usar SimulatedLimitPostOnlyOrderBook
    o SimulatedLimitGTCOrderBook."""
...
class SimulatedLimitPostOnlyOrderBook(BaseSimulatedOrderBook):
    """
    Modo POST-ONLY: Simula órdenes límite Post-Only contra el rango de una vela.

    Reglas:
      1. Post-Only check (maker forzado):
...
class SimulatedLimitGTCOrderBook(BaseSimulatedOrderBook):
    """
    ...
    def gtc_stats(self) -> dict:
        return {
            ...
            "ambiguous_fills": self.ambiguous_fills_count,
        }
...
def build_order_book(commission_pct: float = None,
                     max_posiciones: int = None) -> BaseSimulatedOrderBook:
    """DEPRECATED: Helper legacy. Mantenido solo por compatibilidad."""
    try:
    ...
    return BaseSimulatedOrderBook(commission_pct=_commission, max_posiciones=_maxpos)