"""
╔══════════════════════════════════════════════════════════════════╗
║  OPTIMIZADOR RSI WILDER  v1.0                                 ║
║  Barrido automático de parámetros para encontrar la mejor      ║
║  configuración de la estrategia RSI Wilder.                    ║
║                                                                ║
║  Uso:  python Optimizador_RSI_Wilder.py                        ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
log = get_logger("optimizador_rsi_wilder")
...
# ── Paralelismo ──────────────────────────────────────────────────────────────
CORES_A_DEJAR_LIBRES = 0        # 0 = usar todos los cores disponibles
                                # 1 = dejar 1 core libre, etc.

# ── Checkpoints (separados por estrategia para evitar colisiones) ────────────
CHECKPOINT_PATH     = "optimizer_checkpoint_rsi_wilder.json"
...
TOP20_JSON_PATH     = "optimizer_top20_rsi_wilder.json"
TOP20_CSV_PATH      = "optimizer_top20_rsi_wilder.csv"
...
class RSIEngineSnapshot:
    """Estado interno del RSIEngine en un momento dado."""
    avg_gain: float
    avg_loss: float
...
class RSICache:
    """
    Precalcula RSI para cada período único sobre todas las velas.
    ...
    def _build_for_period(self, period: int) -> None:
        ...
        for c in self.candles:
            ...
            snaps.append(RSIEngineSnapshot(
                avg_gain=engine._avg_gain,
                avg_loss=engine._avg_loss,
    ...
    def restore_engine(self, period: int, idx: int, engine) -> bool:
        """
        Restaura el estado de un RSIEngine al snapshot en la posición idx.
        Retorna True si se pudo restaurar, False si no.
        """
        snap = self.get_snapshot(period, idx)
        if snap is None:
            return False
        engine._avg_gain = snap.avg_gain
        engine._avg_loss = snap.avg_loss
...
class CachedRSIStrategy(RSIWilderStrategy):
    """
    Estrategia RSI que usa valores pre-computados en lugar de recalcular.
    Mantiene la misma lógica de señales que RSIWilderStrategy.
    """
    ...
    def on_candle(self, candle: Candle, wallet) -> List[Signal]:
        ...
        La lógica es idéntica a RSIWilderStrategy.on_candle().
...
class CheckpointManager:
    ...
    def _signal_handler(self, signum, frame) -> None:
        ...
        print(f"  Para reanudar: python Optimizador_RSI_Wilder.py")
        sys.exit(0)
...
def worker_chunk(chunk_data: tuple) -> List[Tuple[Dict, Dict, float]]:
    ...
    for params in combos:
        ...
        strategy = CachedRSIStrategy(
            ...
            # (avg_gain/avg_loss) coincida EXACTAMENTE con el backtest individual.
...
def main() -> None:
    ...
    print("╔" + "═" * 70 + "╗")
    print("║" + "   OPTIMIZADOR RSI WILDER v1.0".ljust(70) + "║")
    print("╚" + "═" * 70 + "╝")
