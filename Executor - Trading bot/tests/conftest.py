...
def pytest_addoption(parser):
    ...
    (Tema #18 de auditoría: tests e2e saltados sin opción CLI para forzarlos)."""
    parser.addoption(
        "--run-network",
        action="store_true",
        default=False,
        help="Ejecutar tests que requieren conexión a red real.",
    )
...
def pytest_configure(config):
    ...
    # Registrar --run-network en la config para acceso global
...
def pytest_collection_modifyitems(config, items):
    ...
    Cuando se pasa --run-network, los tests se ejecutan y si fallan por claves
    faltantes o conexión, el test mismo hará pytest.skip() con mensaje informativo.
    (Tema #18 de auditoría)."""
    if config.getoption("--run-network"):
        return  # No skipear nada, los tests manejarán sus propios fallos
...
def long_candle_sequence():
    """Secuencia de 200 velas para tests de BB y backtesting."""
    return make_candle_sequence(n=200, base_price=30_000.0, volatility=1000.0)


@pytest.fixture
def memory_wallet():
    """Wallet en memoria con 1000 USD y 3 posiciones máximas."""
...
@pytest.fixture
def simulated_ob():
    """OrderBook simulado con 0.1% de comisión y 3 posiciones máx."""
...
@pytest.fixture
def bb_engine():
    """Motor de BB con período 5 y 2.0 std."""
    return BollingerBandsEngine(period=5, std_mult=2.0)


# ══════════════════════════════════════════════════════════════════════════════
# FIXTURES — Estrategia RSI Wilder (portátil desde Backtesting)
# ══════════════════════════════════════════════════════════════════════════════

# NOTA: Las estrategias Bollinger Dual Band fueron ELIMINADAS.
# La estrategia activa es RSI Wilder (LONG + SHORT), portátil desde Backtesting.

@pytest.fixture
def rsi_strategy():
    """Estrategia RSI Wilder preconfigurada (modo offline)."""
...
@pytest.fixture
def rsi_strategy_with_wallet(rsi_strategy, memory_wallet):
    """Estrategia RSI inicializada con wallet."""
    rsi_strategy.on_start(memory_wallet)
    return rsi_strategy, memory_wallet


def make_rsi_signal_candles(
    n_warmup: int = 20,
    direction: str = "short",
    seed: int = 200,
) -> List[Candle]:
    """
    Genera velas que llevan el RSI a zona de sobrecompra (short) o
    sobreventa (long), garantizando una señal de apertura.
    """
    base = make_candle_sequence(n=n_warmup, base_price=30_000.0, seed=seed)
    candles = list(base)

    # Añadir velas en la dirección deseada para mover el RSI
    ts = candles[-1].ts + 3600
    price = candles[-1].close
    step = 200.0 if direction == "short" else -200.0
    for _ in range(15):
        price += step
        candles.append(Candle(
            ts=ts, open=price, high=price + 50, low=price - 50,
