"""
actors/hyperliquid_testnet/hyperliquid_testnet_wallet.py — Billetera sincronizada con Hyperliquid Testnet
══════════════════════════════════════════════════════════════════════════════════════════════════════════
Implementación completa para el entorno Hyperliquid Testnet.
Usa API testnet y claves HL_TESTNET_* desde .env.
"""
...
log = get_logger("hyperliquid_testnet_wallet")

TESTNET_API = constants.TESTNET_API_URL


class HyperliquidWallet(JSONWallet, AsyncWallet):
    """Billetera sincronizada con Hyperliquid Testnet."""
    ...
    def __init__(self, usd_initial: float, max_posiciones: int,
        ...
        self._info = None
        self._last_account_value: float | None = None  # accountValue real del exchange
        log.info("HyperliquidWallet Testnet inicializado", address=account_address)

    def _get_info(self):
        if self._info is None:
            from hyperliquid.info import Info
            self._info = Info(TESTNET_API, skip_ws=True)
        return self._info

    @classmethod
    def from_account(cls, max_posiciones: int, json_path: str,
                     slot_factor: float = 1.0) -> "HyperliquidWallet":
        """Crea la wallet sincronizando el saldo desde Hyperliquid testnet."""
        from support.secrets import secrets
        account_address = secrets("HL_TESTNET_ACCOUNT_ADDRESS")

        from hyperliquid.info import Info
        info = Info(TESTNET_API, skip_ws=True)
        ...
        log.info("Cuenta Hyperliquid Testnet sincronizada",
        ...
        if wallet._slots_used == 0 and pos_size > 0:
            ...
            log.info("Posición huérfana recuperada en Hyperliquid testnet",
        ...
        if wallet._slots_used > 0 and pos_size <= 0:
            ...
            log.warning("Posición fantasma eliminada: existía en JSON local pero no en Hyperliquid testnet")
    ...
    def sync_with_api_sync(self) -> None:
        ...
        try:
            ...
            log.info("Saldo Hyperliquid sincronizado testnet",
                     usd=round(self._usd, 4),
                     slots_used=self._slots_used)
        except Exception as e:
            log.warning("Error sincronizando saldo Hyperliquid Testnet ", error=str(e))
    ...
    async def sync_with_api_async(self, session: aiohttp.ClientSession) -> dict | None:
        ...
        try:
            ...
            log.warning("Error en sync_with_api_async Hyperliquid Testnet", error=str(e))
            return None

    def _apply_sync_state(self, user_state: dict, hl_symbol: str) -> dict | None:
        """
        Reconciliación completa con Hyperliquid Testnet exchange:
        ...
        if abs(self._usd - old_usd) > 0.01 or self._slots_used != old_slots_used:
            ...
            log.debug(
                "Slots recalculados post-sync testnet",
    ...
    def account_value(self) -> float | None:
        """Valor nativo `accountValue` reportado por Hyperliquid testnet, si ya fue sincronizado."""
    ...
    def get_open_positions(self) -> list:
        ...
        try:
            ...
            log.warning("Error obteniendo posiciones Hyperliquid testnet", error=str(e))
            return []
    ...
    async def get_open_positions_async(self, session: aiohttp.ClientSession) -> list:
        ...
        try:
            ...
            log.warning("Error en get_open_positions_async Hyperliquid testnet", error=str(e))
            return []