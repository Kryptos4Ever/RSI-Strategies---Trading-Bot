"""
actors/hyperliquid_mainnet/hyperliquid_mainnet_feed.py — Feed de precios para Hyperliquid Perps
════════════════════════════════════════════════════════════════════════════════════════════
Implementación completa para el entorno Hyperliquid Perps (mainnet).
...
log = get_logger("hyperliquid_mainnet_feed")

HL_MAINNET_API_URL = constants.MAINNET_API_URL
HL_MAINNET_WS_URL = constants.MAINNET_API_URL.replace("https://", "wss://") + "/ws"
...
class HyperliquidRESTFeed(PriceFeed):
    """Feed histórico desde API REST de Hyperliquid Mainnet."""
    ...
    def __init__(self, interval: str | None = None) -> None:
        ...
        log.info("HyperliquidRESTFeed Mainnet inicializado", interval=self.CANDLE_INTERVAL)

    def _get_info(self):
        if self._info is None:
            from hyperliquid.info import Info
            self._info = Info(HL_MAINNET_API_URL, skip_ws=True)
        return self._info
    ...
    def get_candles(self, start: int, end: int, symbol: str = "BTCUSDT") -> List[Candle]:
        ...
        while start_ms < end_ms:
            try:
                ...
                log.error("Error obteniendo velas Hyperliquid Mainnet", error=str(e), exc_info=True)
                break
            if not raw:
                break
            for k in raw:
        ...
        log.info("Velas Hyperliquid Mainnet cargadas", symbol=hl_sym, total=len(all_candles))
        return all_candles
        ...
        while start_ms < end_ms:
            ...
            try:
                async with session.post(url, json=payload) as resp:
                    if resp.status == 429:
                        log.warning("Rate limit Hyperliquid Mainnet, esperando", pause_s=RATE_LIMIT_PAUSE)
...
class HyperliquidWSFeed(PriceFeed, AsyncFeed):
    """Feed de tiempo real como AsyncIterator para Hyperliquid Mainnet."""
    ...
    def __init__(self) -> None:
        ...
        log.info("HyperliquidWSFeed Mainnet inicializado")
    ...
    async def stream(
        ...
        while self._running:
            try:
                log.info("Conectando WS Hyperliquid Mainnet", attempt=attempt)
                async with websockets.connect(
                    HL_MAINNET_WS_URL, ping_interval=30, ping_timeout=10, close_timeout=5,
                ) as ws:
                    await ws.send(subscribe_msg)
                    await ws.send(subscribe_msg_l2)
                    attempt = 0
                    last_tick = _time.time()
                    log.info("WS Hyperliquid Mainnet conectado", symbol=hl_sym)
                    ...
                    while self._running:
                        ...
                        if elapsed > WATCHDOG_TIMEOUT:
                            log.warning("Watchdog Hyperliquid Mainnet: Feed Zombie", elapsed_s=f"{elapsed:.0f}")
                            ka_task.cancel()
                            break
                        try:
                ...
                log.warning("WS Hyperliquid caído, REST Fallback", error=str(e),
    ...
    def subscribe_order_updates(self, account_address: str) -> None:
        ...
        self._user_events_task = loop.create_task(
            ...
            name="hl_mainnet_user_events",
        )
        log.info("Tarea userEvents Hyperliquid Mainnet lanzada", address=account_address[:8] + "...")
    ...
    async def _run_user_events(self, account_address: str, on_order_fill_cb=None) -> None:
        ...
        while self._running:
            try:
                log.info("Conectando userEvents WS Hyperliquid Mainnet", attempt=attempt)
                async with websockets.connect(HL_MAINNET_WS_URL, ping_interval=30, ping_timeout=10) as ws:
                    await ws.send(subscribe_msg)
                    attempt = 0
                    log.info("userEvents WS Hyperliquid Mainnet conectado")
                    async for raw in ws:
                        if not self._running:
                            break
                        try:
                ...
                attempt += 1
                delay = min(2.0 ** attempt, MAX_BACKOFF) + random.uniform(0, 1.0)
                log.warning("userEvents WS Hyperliquid Mainnet caído, reconectando", error=str(e), delay=f"{delay:.1f}")
    ...
    def _parse_fill(self, fill: dict) -> Optional[dict]:
        ...
        try:
            ...
            log.warning("Error parseando fill Hyperliquid Mainnet", error=str(e))
            return None
    ...
    def _parse_candle_from_msg(self, data: dict, hl_sym: str) -> Tuple[Optional[Candle], bool]:
        ...
        try:
            ...
            log.warning("Mensaje WS Hyperliquid Mainnet formato inesperado", error=str(e))
            return None, False

    def stop(self) -> None:
        self._running = False
        log.info("HyperliquidWSFeed Mainnet detenido")