"""
actors/hyperliquid_mainnet/hyperliquid_mainnet_order_book.py — Libro de órdenes para Hyperliquid Perps
═════════════════════════════════════════════════════════════════════════════════════════════════════
Implementación completa para el entorno Hyperliquid Mainnet.
Usa hyperliquid.xyz y claves HL_* desde .env.
...
Configuración en .env:
  HL_ACCOUNT_ADDRESS=0x...   # wallet address (público)
  HL_SECRET_KEY=0x...        # clave privada (API wallet)
...
log = get_logger("hyperliquid_mainnet_order_book")
...
class HyperliquidOrderBook(AsyncOrderBook):
    """
    Libro de órdenes real en Hyperliquid Mainnet.
    ...
    def __init__(
        ...
        log.info(
            "HyperliquidOrderBook Mainnet inicializado",
            symbol=self._symbol,
            leverage=leverage,
    ...
    def _get_exchange(self):
        if self._exchange is None:
            ...
            secret_key = secrets("HL_SECRET_KEY")
            ...
            log.info("HyperliquidExchange Mainnet conectado", address=self._account_address)
    ...
    def _get_account_address(self) -> str:
        ...
        if not self._account_address:
            from support.secrets import secrets
            self._account_address = secrets("HL_ACCOUNT_ADDRESS")
        ...
        try:
            ...
            for asset in meta.get("universe", []):
                if asset.get("name") == self._symbol:
                    ...
                    log.info("szDecimals obtenido Mainnet", symbol=self._symbol, sz_decimals=self._sz_decimals)
                    return self._sz_decimals
            log.warning("Asset no encontrado en meta Mainnet — fallback 5", symbol=self._symbol)
        except Exception as e:
            log.warning("Error obteniendo szDecimals Mainnet — fallback 5", error=str(e))
        self._sz_decimals = 5
        return self._sz_decimals
    ...
    def _ensure_leverage(self, exchange) -> None:
        if not self._leverage_applied:
            try:
                ...
                log.info("Leverage aplicado Mainnet", symbol=self._symbol, leverage=self._leverage)
            except Exception as e:
                log.warning("Error aplicando leverage Mainnet", error=str(e))
    ...
    def _execute_buy_limit(self, exchange, order: Order) -> bool:
        ...
        if reduce_only:
            ...
            if qty * px < 10.0:
                ...
                log.warning("Orden rechazada: nocional menor al mínimo Mainnet", symbol=self._symbol, notional=round(qty * px, 2))
                return False
    ...
    def _parse_limit_response(self, order: Order, response: dict, is_buy: bool, qty: float) -> bool:
        ...
        for s in statuses:
            if "error" in s:
                log.warning("Orden Hyperliquid rechazada por exchange",
                ...
                log.info(
                    "Orden límite con fill inmediato Mainnet",
                    side="BUY" if is_buy else "SELL",
                    symbol=self._symbol,
        ...
        log.warning("Respuesta Hyperliquid Mainnet sin statuses reconocidos", data=str(data)[:200])
    ...
    async def cancel_all_async(self, session=None) -> int:
        ...
        try:
            ...
            if not oids_to_cancel:
                log.debug("No había órdenes abiertas para cancelar Mainnet")
                return 0

            # Ejecutar cancelación
            exchange.bulk_cancel(oids_to_cancel)
            log.info(
                "Órdenes límite canceladas al cambio de vela Mainnet",
            ...
            log.warning("Error en cancel_all_async Mainnet", error=str(e))
            return 0
    ...
    async def get_open_order_oids(self, session=None) -> set:
        ...
        try:
            ...
            log.warning("Error obteniendo órdenes abiertas Mainnet", error=str(e))
            return set()
    ...
    def set_dead_mans_switch(self, cancel_at_ms: int | None) -> None:
        ...
        try:
            if self._dead_mans_switch_initialized and cancel_at_ms == self._last_dead_mans_switch_ms:
                log.debug("Dead Man's Switch sin cambios Mainnet", cancel_at_ms=cancel_at_ms)
                return
            exchange = self._get_exchange()
            ...
            if cancel_at_ms:
                log.info("Dead Man's Switch activado Mainnet", cancel_at_ms=cancel_at_ms)
            else:
                log.info("Dead Man's Switch desactivado Mainnet")
        except Exception as e:
            log.warning("Error configurando Dead Man's Switch Mainnet", error=str(e))
    ...
    async def submit_bulk_async(
        ...
        try:
            ...
            log.info(
                "Bulk order enviado Mainnet",
                n_orders=len(orders_payload),
                symbol=self._symbol,
            )
        except Exception as e:
            log.error("Error en submit_bulk_async Mainnet", error=str(e))
            for ord_ in targets:
        ...
        for attempt, delay in enumerate(RETRY_DELAYS + [None]):
            try:
                ...
                if order.reject_reason and any(
                    ...
                    log.warning("Error no retriable, abortando reintento Mainnet",
                ...
                last_exc = e
                log.warning(
                    f"Error en orden Hyperliquid Perps (intento {attempt + 1})",
                    error=str(e),
