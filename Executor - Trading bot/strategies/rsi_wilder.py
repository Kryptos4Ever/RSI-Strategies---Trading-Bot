"""
strategies/rsi_wilder.py — Estrategia RSI Wilder
═══════════════════════════════════════════════════════════════════
...
Los precios se calculan con RSIEngine.price_for_rsi() (Wilder inverso).
"""
...
log = get_logger("rsi_wilder")


class RSIWilderStrategy(BaseStrategy):
    """
    Estrategia de Mean Reversion basada en RSI (Wilder's Smoothing) con precios calculados.
    ...
    def __init__(
        ...
        self._rsi_engine = RSIEngine(period=rsi_period)
        ...
        # precios límite derivados (Wilder inverso).
    ...
    def load_warmup(self, candles: List[Candle]) -> None:
        """
        Carga velas previas al rango para inicializar el RSI (Wilder's Smoothing).
    ...
    def on_candle(self, candle: Candle, wallet) -> List[Signal]:
        ...
        if rsi < 50.0:
            # Precio al que se abriría LONG (precio correspondiente a oversold=30)
            price_open = self._rsi_engine.price_for_rsi(self._oversold)
            # Precio de reducción LONG (autónomo vía reduce_long, calculado con Wilder inverso)
        ...
        # ── ZONA SHORT: RSI > 50 (punto neutral de la media) ──────────────
            ...
            # Precio al que se abriría SHORT (precio correspondiente a overbought=70)
            price_open = self._rsi_engine.price_for_rsi(self._overbought)
            # Precio de reducción SHORT (autónomo vía reduce_short, calculado con Wilder inverso)
    ...
    def compute_overlay_row(self, candle: Candle) -> dict:
        """
        Calcula los 4 niveles RSI (oversold, overbought, reduce_long, reduce_short)
        para la vela actual SIN mutar el estado de la estrategia.

        Este método lo usa el LiveEngine para capturar los overlays al INICIO
        de cada vela (antes de que se desarrolle), usando el RSI vigente
        (closes hasta la vela anterior). No requiere la wallet ni modifica
        buffers: es puramente de lectura.

        Devuelve {id_overlay: valor} para fusionar inline en la vela del JSON.
        """
        rsi = self._rsi_engine.value
        if rsi is None:
            return {}
        return {
            "oversold":     self._rsi_engine.price_for_rsi(self._oversold),
            "overbought":   self._rsi_engine.price_for_rsi(self._overbought),
            "reduce_long":  self._rsi_engine.price_for_rsi(self._reduce_long),
            "reduce_short": self._rsi_engine.price_for_rsi(self._reduce_short),
        }

    def on_stop(self, wallet) -> None:
        """Cierre de estrategia: log final."""
        log.debug("RSI Wilder finalizada",
    ...
    def get_chart_overlay_row(self, candle_ts) -> dict:
        ...
        Cada id como clave y su precio calculado (Wilder inverso) como valor.
