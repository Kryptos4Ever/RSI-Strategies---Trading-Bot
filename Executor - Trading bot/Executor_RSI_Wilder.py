#!/usr/bin/env python3
"""
Executor_RSI_Wilder.py — Lanzador rápido para la estrategia RSI Wilder en vivo
═══════════════════════════════════════════════════════════════════════════════════════════════
Configura y ejecuta la estrategia RSI Wilder (LONG + SHORT) en el entorno de ejecución
en vivo. Acepta argumentos CLI que sobreescriben las constantes de configuración.
...
log = get_logger("executor_rsi_wilder_runner")
...
STRATEGY_REGISTRY = {
    "rsi": "strategies.rsi_wilder:RSIWilderStrategy",
}
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Executor RSI Mean Reversion — Lanzador rápido de la estrategia RSI en vivo",
...
def main() -> None:
    ...
    strategy_class = _load_strategy_class("rsi")
    ...
    state_path = f"live_results_wilder_{environment}.json"
